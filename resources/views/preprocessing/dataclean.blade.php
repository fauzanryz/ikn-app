         ...
         <div class="modal-content">
          <form action="{{ route('preprocessing.updateSentimen', $row->id) }}" method="POST">
           ...
           <div class="modal-header">
            <h5 class="modal-title" id="editModalLabel">Edit Sentimen</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
           </div>
           <div class="modal-body">
            <div class="mb-3">
             <label for="sentimen" class="form-label">Sentimen</label>
             <select class="form-select" name="sentimen" required>
              <option value="positif" {{ $row->sentimen == 'positif' ? 'selected' : '' }}>Positif</option>
              <option value="negatif" {{ $row->sentimen == 'negatif' ? 'selected' : '' }}>Negatif</option>
             </select>
            </div>
           </div>
